...
GlobalRouteManagerImpl::GlobalRouteManagerImpl()
    : m_spfroot(nullptr),
      m_useBmssp(true)  // Default: Dijkstra (switched via make breaking/dijkstra)
{
    NS_LOG_FUNCTION(this);
...
GlobalRouteManagerImpl::SPFCalculate(Ipv4Address root)
{
    ...
    if (m_useBmssp)
    {
        NS_LOG_LOGIC("[Switched] Using BMSSP Algorithm for root: " << root);
        SPFCalculateBMSSP(root);
        return; // 运行完新算法直接返回，跳过后面原有的 Dijkstra
    }
    // =======================================================
...
// [新增] 在文件末尾添加
void
GlobalRouteManagerImpl::SPFCalculateBMSSP(Ipv4Address root)
{
    NS_LOG_FUNCTION(this << root);

    // ================== BMSSP 集成测试（仅在详细日志模式下显示）==================
    // 注释掉测试代码以减少日志输出，如需调试可取消注释
    /*
    NS_LOG_LOGIC("--- Starting BMSSP Calculation Check ---");
    BmsspSolver testSolver;
    testSolver.Init(3);
    testSolver.AddEdge(0, 1, 10);
    testSolver.AddEdge(1, 2, 20);
    testSolver.Run(0);
    BmsspLength distToNode2 = testSolver.GetDistance(2);
    NS_LOG_LOGIC("BMSSP: Test Run Completed. Distance from node 0 to node 2 is: " << distToNode2);
    if (distToNode2 == 30)
    {
        NS_LOG_LOGIC("BMSSP: Test PASSED!");
    }
    else
    {
        NS_LOG_WARN("BMSSP: Test FAILED! Expected distance 30, got " << distToNode2);
    }
    */
    // ================== BMSSP 集成测试结束 ==================
    ...
    // 第一步：收集所有路由器节点，建立地址到索引的映射
    std::map<Ipv4Address, int> addrToIndex;
    std::vector<Ipv4Address> indexToAddr;
    int nodeIndex = 0;
    ...
    for (auto i = NodeList::Begin(); i != NodeList::End(); i++)
    {
        ...
        Ipv4Address routerId = router->GetRouterId();
        if (addrToIndex.find(routerId) == addrToIndex.end())
        {
            addrToIndex[routerId] = nodeIndex;
            indexToAddr.push_back(routerId);
            nodeIndex++;
        }
    }

    int numNodes = nodeIndex;
    NS_LOG_LOGIC("Found " << numNodes << " routers");

    if (numNodes == 0)
    {
        NS_LOG_WARN("No routers found, skipping BMSSP calculation");
        return;
    }

    // 检查 root 是否在映射中
    if (addrToIndex.find(root) == addrToIndex.end())
    {
        NS_LOG_WARN("Root " << root << " not found in router list");
        return;
    }

    int rootIndex = addrToIndex[root];

    // 第二步：创建 BmsspSolver 并初始化
    BmsspSolver solver;
    solver.Init(numNodes);
    ...
    for (auto i = NodeList::Begin(); i != NodeList::End(); i++)
    {
        ...
        Ipv4Address routerId = router->GetRouterId();
        GlobalRoutingLSA* lsa = m_lsdb->GetLSA(routerId);
        if (!lsa)
        {
            continue;
        }

        int u = addrToIndex[routerId];

        // 遍历该 LSA 的所有链接记录
        for (uint32_t j = 0; j < lsa->GetNLinkRecords(); j++)
        {
            GlobalRoutingLinkRecord* lr = lsa->GetLinkRecord(j);
            ...
            if (lr->GetLinkType() == GlobalRoutingLinkRecord::PointToPoint)
            {
                Ipv4Address linkId = lr->GetLinkId();
                uint32_t metric = lr->GetMetric();

                // 查找目标路由器
                // 注意：这里只需要添加单向边 u -> v
                // 因为当你遍历到节点 v 的时候，会自动添加 v -> u 的边
                // 如果在这里同时添加两条，会导致图里的边数量翻倍，虽然不影响最短路结果，但浪费内存
                
                if (addrToIndex.find(linkId) != addrToIndex.end())
                {
                    int v = addrToIndex[linkId];
                    
                    // [修改后] 只添加单向边
                    solver.AddEdge(u, v, (int)metric); 
                    
                    NS_LOG_LOGIC("Added edge: " << routerId << " -> " << linkId 
                                << " (metric: " << metric << ")");
                }
            }
        }
    ...
    // 第四步：运行 BMSSP 算法
    NS_LOG_LOGIC("Running BMSSP algorithm from root: " << root << " (index: " << rootIndex << ")");
    solver.Run(rootIndex);

    // 第五步：将结果转换回 NS-3 的路由表
    // 获取 root 节点对应的 NS-3 Node 对象
    Ptr<Node> rootNode = nullptr;
    for (auto i = NodeList::Begin(); i != NodeList::End(); i++)
    {
        Ptr<Node> node = *i;
        Ptr<GlobalRouter> router = node->GetObject<GlobalRouter>();
        if (router && router->GetRouterId() == root)
        {
            rootNode = node;
            break;
        }
    ...
    if (!rootNode)
    {
        NS_LOG_WARN("Cannot find root node for " << root);
        return;
    }
    
    Ptr<GlobalRouter> rootRouter = rootNode->GetObject<GlobalRouter>();
    NS_ASSERT(rootRouter);
    Ptr<Ipv4GlobalRouting> rootGr = rootRouter->GetRoutingProtocol();
    NS_ASSERT(rootGr);
    
    // 获取 root 的 LSA，用于查找直连出接口
    GlobalRoutingLSA* rootLsa = m_lsdb->GetLSA(root);
    if (!rootLsa)
    {
        NS_LOG_WARN("Cannot find LSA for root " << root);
        return;
    }
    
    NS_LOG_LOGIC("BMSSP calculation completed. Building routing table...");
    
    // 辅助函数：查找从 sourceNode 到 nextHopNodeId 的连接接口 IP
    // 返回找到的 Gateway IP 和出接口索引，如果找不到则返回 false
    auto FindNextHopIP = [&](Ipv4Address sourceAddr, Ipv4Address nextHopAddr, 
                              Ipv4Address& gatewayIP, int32_t& outIf) -> bool {
        // 获取 sourceNode 的 LSA
        GlobalRoutingLSA* sourceLsa = m_lsdb->GetLSA(sourceAddr);
        if (!sourceLsa)
        {
            return false;
        }
        
        // 遍历 sourceNode 的所有邻居（通过 LSA 链接记录）
        for (uint32_t j = 0; j < sourceLsa->GetNLinkRecords(); j++)
        {
            GlobalRoutingLinkRecord* lr = sourceLsa->GetLinkRecord(j);
            if (lr->GetLinkType() == GlobalRoutingLinkRecord::PointToPoint)
            {
                Ipv4Address linkId = lr->GetLinkId();
                // 检查这个链接是否连接到 nextHopNodeId
                if (linkId == nextHopAddr)
                {
                    // 找到了连接到 nextHopNodeId 的链接
                    // 现在需要从 nextHopNodeId 的 LSA 中找到指向 sourceNode 的反向链接
                    // 反向链接的 LinkData 就是 nextHop 的接口 IP（Gateway IP）
                    GlobalRoutingLSA* nextHopLsa = m_lsdb->GetLSA(nextHopAddr);
                    if (!nextHopLsa)
                    {
                        return false;
                    }
                    
                    // 在 nextHop 的 LSA 中查找指向 sourceNode 的链接记录
                    for (uint32_t k = 0; k < nextHopLsa->GetNLinkRecords(); k++)
                    {
                        GlobalRoutingLinkRecord* reverseLr = nextHopLsa->GetLinkRecord(k);
                        if (reverseLr->GetLinkType() == GlobalRoutingLinkRecord::PointToPoint)
                        {
                            Ipv4Address reverseLinkId = reverseLr->GetLinkId();
                            if (reverseLinkId == sourceAddr)
                            {
                                // 找到了反向链接！LinkData 就是 nextHop 的接口 IP
                                gatewayIP = reverseLr->GetLinkData();
                                
                                // 获取出接口：从 sourceNode 的链接记录中获取 LinkData（sourceNode 的本地接口 IP）
                                Ipv4Address sourceLocalIP = lr->GetLinkData();
                                
                                // 找到 sourceNode 对应的 Node 对象
                                Ptr<Node> sourceNode = nullptr;
                                for (auto iter = NodeList::Begin(); iter != NodeList::End(); iter++)
                                {
                                    Ptr<Node> node = *iter;
                                    Ptr<GlobalRouter> router = node->GetObject<GlobalRouter>();
                                    if (router && router->GetRouterId() == sourceAddr)
                                    {
                                        sourceNode = node;
                                        break;
                                    }
                                ...
                                // 通过本地接口 IP 查找出接口索引
                                outIf = ipv4->GetInterfaceForPrefix(sourceLocalIP, Ipv4Mask("255.255.255.255"));
                                if (outIf >= 0)
                                {
                                    return true;
                                }
    ...
    for (int i = 0; i < numNodes; i++)
    {
        ...
        BmsspLength dist = solver.GetDistance(i);
        if (dist == BMSSP_INF)
        {
            continue; // 不可达，跳过
        }
        ...
        // 获取下一跳节点ID
        int nextHopIndex = solver.GetNextHop(i);
        if (nextHopIndex == -1)
        {
            NS_LOG_WARN("Cannot find next hop for node " << indexToAddr[i]);
            continue;
        }
        
        Ipv4Address targetAddr = indexToAddr[i];
        Ipv4Address nextHopAddr = indexToAddr[nextHopIndex];
        
        // 【关键修复】查找从 root 到 nextHop 的连接接口 IP
        // 如果 nextHop 是 root 的直连邻居，直接查找
        // 如果 nextHop 不是 root 的直连邻居，需要递归查找路径上的第一个节点
        Ipv4Address gatewayIP;
        int32_t outIf = -1;
        
        // 首先检查 nextHop 是否是 root 的直连邻居
        if (FindNextHopIP(root, nextHopAddr, gatewayIP, outIf))
        {
            // nextHop 是 root 的直连邻居，已经找到了 Gateway IP 和出接口
            NS_LOG_LOGIC("Next hop " << nextHopAddr << " is direct neighbor of root " << root
                        << ", gateway IP: " << gatewayIP << ", outIf: " << outIf);
        }
        else
        {
            // nextHop 不是 root 的直连邻居，需要递归查找路径
            // 从 nextHop 开始，沿着路径向上查找，直到找到 root 的直连邻居
            int currentIndex = nextHopIndex;
            bool found = false;
            
            // 沿着路径向上查找，直到找到 root 的直连邻居
            while (currentIndex != rootIndex && !found)
            {
                int parentIndex = solver.GetNextHop(currentIndex);
                if (parentIndex == -1 || parentIndex == currentIndex)
                {
                    NS_LOG_WARN("Cannot find path from root " << root 
                                << " to next hop " << nextHopAddr);
                    break;
                }
                
                // 如果父节点是 root，说明 current 是 root 的直连邻居
                if (parentIndex == rootIndex)
                {
                    Ipv4Address currentNextHopAddr = indexToAddr[currentIndex];
                    if (FindNextHopIP(root, currentNextHopAddr, gatewayIP, outIf))
                    {
                        // 找到了！current 是 root 的直连邻居
                        // gatewayIP 和 outIf 已经设置好了
                        NS_LOG_LOGIC("Found direct neighbor " << currentNextHopAddr 
                                    << " on path from root " << root << " to target " << targetAddr
                                    << ", gateway IP: " << gatewayIP << ", outIf: " << outIf);
                        found = true;
                        break;
                    }
            ...
            if (!found)
            {
                NS_LOG_WARN("Cannot find direct neighbor on path from root " << root 
                            << " to next hop " << nextHopAddr << " for target " << targetAddr);
            }
        }
        
        if (outIf < 0)
        {
            NS_LOG_WARN("Cannot find outgoing interface from root " << root 
                        << " to next hop " << nextHopAddr << " for target " << targetAddr);
            continue;
        }
        
        // 获取目标节点的 LSA，用于添加路由
        GlobalRoutingLSA* targetLsa = m_lsdb->GetLSA(targetAddr);
        if (!targetLsa)
        {
            continue;
        }
        
        // 添加路由：为目标节点的所有点对点链接添加主机路由
        // 这类似于 SPFIntraAddRouter 的逻辑
        for (uint32_t j = 0; j < targetLsa->GetNLinkRecords(); j++)
        {
            GlobalRoutingLinkRecord* lr = targetLsa->GetLinkRecord(j);
            if (lr->GetLinkType() == GlobalRoutingLinkRecord::PointToPoint)
            {
                // 添加主机路由到目标节点的本地接口地址
                // 使用找到的真实 Gateway IP 地址
                rootGr->AddHostRouteTo(lr->GetLinkData(), gatewayIP, outIf);
                NS_LOG_LOGIC("Node " << rootNode->GetId() 
                            << " adding host route to " << lr->GetLinkData()
                            << " using next hop " << gatewayIP
                            << " via interface " << outIf);
            }
        }
    }
    
    // 处理 Stub 网络（简化版本，先跳过，后续可以完善）
    // SPFProcessStubs 需要 SPFVertex 树，暂时跳过
    NS_LOG_LOGIC("BMSSP routing table construction completed for root: " << root);
}
