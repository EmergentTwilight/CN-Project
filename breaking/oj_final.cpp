...
struct Block {
    ...
    Block() : head(nullptr), tail(nullptr), size(0), upperBound(V()), owns_nodes(true) {}
...
public:
    BlockHeapDS(int m, V B, int n) : M(m), globalB(B), N(n), D1TotalSize(0) {}

    ~BlockHeapDS() {
        for (auto b : D0) delete b;
        for (auto b : D1) delete b;
    }
    ...
    void Split(int blockIdx) {
        if (blockIdx >= (int)D1.size()) return;
        Block<K, V>* block = D1[blockIdx];
        if (block->size <= M) return;
    ...
    bool IsEmpty() const {
        for (auto b : D0) if (b->size > 0) return false;
        for (auto b : D1) if (b->size > 0) return false;
        return true;
    }
};
...
std::pair<std::vector<Vertex>, std::vector<Vertex>> FindPivots(Length B, const std::vector<Vertex>& S)
{
...
std::pair<Length, std::vector<Vertex>> BaseCase(Length B, const std::vector<Vertex>& S)
{
    assert(S.size() == 1);
    Vertex x = S[0];
...
// ============ BMSSP - Algorithm 3 ============
std::pair<Length, std::vector<Vertex>> BMSSP(int level, Length B, const std::vector<Vertex>& S)
{
    if (level == 0) {
        return BaseCase(B, S);
    }
